// Admin UI layer — Notifications tab (simulated admin inbox)
// Round 2: owns its data via useAdminNotifications (instantiated here, not in the
// orchestrator). The tab only mounts when active, so the inbox is read once on
// mount instead of re-parsing localStorage on every activeTab change.
import { memo } from 'react';
import { Mail, Trash2 } from 'lucide-react';
import { useAdminNotifications } from '../hooks/useAdminNotifications';

function NotificationsView({ notifications, selectedNotification, onSelect, onClear }) {
  return (
              ...
              {notifications.map((notif) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-slate-400 mt-1 line-clamp-2">To: {notif.recipient}</p>
                  </div>
                );
              })}
            </div>
...
const MemoNotificationsView = memo(NotificationsView);

export default function NotificationsTab() {
  const { notifications, selectedNotification, setSelectedNotification, clearInbox } = useAdminNotifications();
  return (
    <MemoNotificationsView
