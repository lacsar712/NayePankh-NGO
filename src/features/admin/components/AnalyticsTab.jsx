// Admin UI layer — Analytics tab (counters, SVG charts, simulator panel)
import { memo, useMemo } from 'react';
import { TrendingUp, Users, Calendar, Heart, Leaf, Utensils, Sparkles } from 'lucide-react';
import { computeDonationTrend, computeTargetProgress } from '../utils/analytics';

function AnalyticsTab({ stats, onFoodDrive, onTreePlanting, onHealthCamp, onCorporateGrant }) {
  const {
    totalDonationSum,
  ...
  // Chart geometry + progress percentages are derived by pure functions and
  // memoized here so the SVG path strings are recomputed only when inputs change.
  const trend = useMemo(() => computeDonationTrend(totalDonationSum), [totalDonationSum]);
  const progress = useMemo(() => computeTargetProgress(stats), [stats]);
    ...
    <div className="space-y-10">
        ...
        <div className="lg:col-span-7 bg-white border border-slate-200/80 p-6 rounded-2xl shadow-sm space-y-4">
          ...
          <div className="h-[220px] w-full pt-4">
            <svg viewBox="0 0 500 200" className="w-full h-full overflow-visible">
              ...
              />
              {trend.points.map(([cx, cy]) => (
                <circle key={cx} cx={cx} cy={cy} r="4.5" fill="white" stroke="rgb(236, 72, 153)" strokeWidth="2.5" />
              ))}
          ...
          <div className="space-y-5">
            <div className="space-y-1.5">
              ...
              <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                <div className="h-full bg-rose-500 rounded-full transition-all duration-500" style={{ width: `${progress.beneficiaries}%` }} />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                <div className="h-full bg-emerald-500 rounded-full transition-all duration-500" style={{ width: `${progress.trees}%` }} />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                <div className="h-full bg-amber-500 rounded-full transition-all duration-500" style={{ width: `${progress.meals}%` }} />
              </div>
            </div>
          </div>
      ...
      <div className="bg-slate-900 border border-slate-800 text-white p-6 sm:p-8 rounded-3xl shadow-xl space-y-6 relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
          <button onClick={onFoodDrive} className="p-4 bg-slate-800 border border-slate-700/60 hover:bg-slate-750 rounded-2xl text-center text-xs font-bold transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-sm flex flex-col items-center justify-center space-y-2 text-white">
            <Utensils className="h-5 w-5 text-amber-400" />
            <span>Food Drive (+500 Meals)</span>
          </button>
          <button onClick={onTreePlanting} className="p-4 bg-slate-800 border border-slate-700/60 hover:bg-slate-750 rounded-2xl text-center text-xs font-bold transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-sm flex flex-col items-center justify-center space-y-2 text-white">
            <Leaf className="h-5 w-5 text-emerald-400" />
            <span>Tree Planting (+100 Trees)</span>
          </button>
          <button onClick={onHealthCamp} className="p-4 bg-slate-800 border border-slate-700/60 hover:bg-slate-750 rounded-2xl text-center text-xs font-bold transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-sm flex flex-col items-center justify-center space-y-2 text-white">
            <Heart className="h-5 w-5 text-rose-400" />
            <span>Health Camp (+250 People)</span>
          </button>
          <button onClick={onCorporateGrant} className="p-4 bg-slate-800 border border-slate-700/60 hover:bg-slate-750 rounded-2xl text-center text-xs font-bold transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-sm flex flex-col items-center justify-center space-y-2 text-white">
