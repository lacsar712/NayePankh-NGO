...
import { EVENT_TYPE_MAP } from '../constants/defaults';

/**
 * 【ui 层】活动管理 Tab（受控展示组件：表单状态由 Hook 持有，通过 props 注入）
 * @param {{
 *   events: Array,
 *   newEvent: object,
 *   onNewEventChange: (next: object) => void,
 *   onAddEvent: (e: {preventDefault: () => void}) => void,
 *   onDeleteEvent: (id: string) => void
 * }} props
 */
function EventsTab({ events, newEvent, onNewEventChange, onAddEvent, onDeleteEvent }) {
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={onAddEvent} className="bg-slate-50 border border-slate-100 p-6 rounded-2xl grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              onNewEventChange({ ...newEvent, rawType: val, type: EVENT_TYPE_MAP[val] });
            }}
            className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-primary-500 transition-colors"
          >
